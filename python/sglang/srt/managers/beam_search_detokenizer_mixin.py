...
from sglang.srt.managers.beam_search_type import BeamSearchOutput
from sglang.srt.managers.io_struct import BatchTokenIDOutput


class BeamSearchDetokenizerMixin:
    def is_beam_search_batch(self, beam_search_output) -> bool:
        """Check if the (already-unwrapped) batch contains beam search requests."""
        return beam_search_output is not None and len(beam_search_output) > 0

    def decode_beam_search_output(
        self, recv_obj: BatchTokenIDOutput, beam_search_output: List[BeamSearchOutput]
    ):
        """Decode beam search candidate sequences to text.

        ``beam_search_output`` is the unwrapped List[BeamSearchOutput] (the
        transport field ``recv_obj.beam_search_output`` is a PickleWrapper).
        """
