...
class ScheduleBatchBeamSearchMixin:
    ...
    def prepare_for_beam_search_decode(self: ScheduleBatch):
        ...
        # Undo the per-req kv_allocated_len bump done inside alloc_for_decode:
        # beam decode writes KV into the per-beam pool rows (freed manually by
        # the beam KV logic), not into the request's original prefill row, so
        # the request-level accounting must stay at the prefill length for
        # release_kv_cache to reclaim only the prefill row.
...
class ReqBeamSearchMixin:
    ...
    def _init_beam_search_attributes(self, is_beam_search, sampling_params):
        ...
        if self.is_beam_search:
            ...
            # Beam search does not support returning logprobs. Force-disable it
            # here so the scheduler's Req construction stays agnostic (this
            # converges the per-call return_logprob/top_logprobs_num overrides
            # that would otherwise live in scheduler.py).
            self.return_logprob = False
            if getattr(self, "logprob", None) is not None:
                self.logprob.top_logprobs_num = 0
