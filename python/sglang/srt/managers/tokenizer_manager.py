...
class TokenizerManager(
    ...
    async def _handle_batch_output(
        ...
        if isinstance(recv_obj, (BatchStrOutput, BatchTokenIDOutput)):
            ...
            # Beam search results are transported as a pickled
            # List[BeamSearchOutput]; unwrap once so per-index access works.
            recv_obj.beam_search_output = unwrap_from_pickle(
                recv_obj.beam_search_output
            )
        else:
